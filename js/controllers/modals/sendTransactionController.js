...
angular.module('lazarusApp').controller('sendTransactionController', ['$scope', 'sendTransactionModal', '$http', 'userService', 'feeService', '$timeout', '$filter', function ($scope, sendTransactionModal, $http, userService, feeService, $timeout, $filter) {
    ...
    $scope.isCorrectValue = function (currency, throwError) {
        ...
        function error () {
            $scope.errorMessage.amount = 'Invalid LAZ amount';
    ...
    $scope.sendTransaction = function (secretPhrase, withSecond) {
        ...
        if (($scope.amount + '').indexOf('.') != -1 && $scope.amount.split('.')[1].length > 8) {
            $scope.errorMessage.amount = 'LAZ amount must not have more than 8 decimal places';
            return;
        }
