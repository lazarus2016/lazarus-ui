...
angular.module('lazarusApp').controller('multisignatureModalController', ["$scope", "$http", "multisignatureModal", "viewFactory", "userService", "feeService", "gettextCatalog", function ($scope, $http, multisignatureModal, viewFactory, userService, feeService, gettextCatalog) {
    ...
    $scope.addMember = function (member) {
        ...
        if ($scope.member.trim() == '') {
            ...
            if (buffer.length == 32) {
                var lazarus = require('lisk-js');
                var address = lazarus.crypto.getAddress($scope.member);
