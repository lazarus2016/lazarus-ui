...
angular.module('lazarusApp').controller('appController', ['dappsService', '$scope', '$rootScope', '$http', "userService", "$interval", "$timeout", 'viewFactory', '$state', 'blockService', 'sendTransactionModal', 'registrationDelegateModal', 'serverSocket', 'delegateService', '$window', 'forgingModal', 'errorModal', 'userInfo', 'transactionsService', 'secondPassphraseModal', 'focusFactory', 'gettextCatalog', function (dappsService, $rootScope, $scope, $http, userService, $interval, $timeout, viewFactory, $state, blockService, sendTransactionModal, registrationDelegateModal, serverSocket, delegateService, $window, forgingModal, errorModal, userInfo, transactionsService, secondPassphraseModal, focusFactory, gettextCatalog) {
    ...
    $scope.getPriceTicker = function () {
        $http.get("https://explorer.lazarus.com/api/getPriceTicker")
            .then(function (response) {
                $scope.btc_usd = response.data.btc_usd;
                $scope.lazarus_btc = response.data.lazarus_btc;
                $scope.lazarus_usd = response.data.lazarus_usd;
            });
    };
    ...
    $scope.getVersion = function () {
        $http.get("/api/peers/version").then(function (response) {
            if (response.data.success) {
                ...
                $http.get("https://login.lazarus.com/api/peers/version").then(function (response) {
    ...
    $scope.convertToUSD = function (lazarus) {
        return (lazarus / 100000000) * $scope.lazarus_usd;
    };
